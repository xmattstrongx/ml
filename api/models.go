...
func LoadConfig(path string) Configuration {
	...
	if err != nil {
		logger.Errorf("Config File Missing error: %+v ", err)
	}
...
func ConvertTimeToISO(tm string) string {
	// if completed is provided alter timestamp to ISO8061
	if tm != "" {
		tt, err := time.Parse("20060102T15:04:05-07:00", tm)
		if err != nil {
			logger.Error("Unable to format provided timestamp provided in completed")
			return "0001-01-01T00:00:00+0000"
		}
		return string(tt.Format("2006-01-02T15:04:05-0700"))
	}
	return "0001-01-01T00:00:00+0000"
}
...
func AttributesToTask(info map[string]*dynamodb.AttributeValue) Task {
	var t Task
	if value, found := info["taskid"]; found {
		t.TaskID = *value.S
	}
	if value, found := info["user"]; found {
		t.User = *value.S
	}
	if value, found := info["description"]; found {
		t.Description = *value.S
	}
	if value, found := info["priority"]; found {
		pr, err := strconv.Atoi(*value.S)
		if err == nil {
			t.Priority = &pr
		}
	}
	if value, found := info["completed"]; found {
		t.Completed = *value.S
	}
	return t
}
